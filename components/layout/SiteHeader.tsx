    ...
    <header className="sticky top-0 z-40 border-b border-paper-300 bg-paper-50/95 backdrop-blur">
      ...
      <div className="container-page flex h-16 items-center justify-between">
        ...
        <nav className="hidden items-center gap-6 md:flex" aria-label="Main">
          {navLinks.map((l) => (
            <Link key={l.href} href={l.href} className="text-sm font-medium text-ink-700 hover:text-terracotta-600">
              {l.label}
            </Link>
          ))}
        </nav>
      ...
      {open && (
        <nav className="border-t border-paper-300 bg-paper-50 md:hidden" aria-label="Mobile">
          <div className="container-page flex flex-col gap-1 py-3">
            {navLinks.map((l) => (
              <Link key={l.href} href={l.href} className="rounded-lg px-2 py-2.5 text-sm font-medium hover:bg-paper-200">
                {l.label}
              </Link>
            ))}
            <Link href="/login" className="rounded-lg px-2 py-2.5 text-sm font-medium hover:bg-paper-200">
              Log in
            </Link>
            <Link href="/register" className="rounded-lg px-2 py-2.5 text-sm font-semibold text-terracotta-600">
              Give or Get Books
            </Link>
          </div>
        </nav>
