...
export const mockAdapter: ApiAdapter = {
  ...
  // ── Admin mock data & implementations ────────────────────────────────────
  ...
  async adminListListings() {
    ...
    const allListings = [...listings, {
      id: "4", owner: owners.u2, title: "Removed: Old Geography Textbook", description: "Removed by moderator.", category: catScience, gradeLevel: null, condition: "worn" as const, status: "removed" as const, images: [], distanceKm: null, createdAt: "2026-05-01T00:00:00Z", updatedAt: "2026-07-01T00:00:00Z",
    }];
    return { results: allListings, nextCursorUrl: null, previousCursorUrl: null };
  },
  async adminRemoveListing(id) {
    await delay();
    listings = listings.map((l) => l.id === id ? { ...l, status: "removed" as const } : l);
  },
  async adminRestoreListing(id) {
    await delay();
    listings = listings.map((l) => l.id === id ? { ...l, status: "available" as const } : l);
  },
