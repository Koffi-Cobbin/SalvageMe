...
export function ListingCard({ listing }: { listing: Listing }) {
  ...
  return (
    <Link href={`/listings/${listing.id}`} className="no-underline flex h-full">
      <article
        className={clsx(
          "group flex h-full w-full flex-col rounded-xl2 border border-paper-300 bg-white transition-shadow",
          "[box-shadow:0_2px_10px_rgba(29,26,21,0.06)] hover:[box-shadow:0_6px_20px_rgba(29,26,21,0.10)]",
        )}
      >
        <div className="relative h-44 w-full flex-shrink-0 overflow-hidden rounded-t-xl2 bg-paper-200">
          {image ? (
            <img
              src={image.url}
              alt={listing.title}
